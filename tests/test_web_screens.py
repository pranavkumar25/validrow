...
SCREENS = [
    ("/", ["Dashboard", "Validation volume", "Verdict mix", "Recent results"]),
    ("/dashboard", ["QUALITY SCORE", "SEGMENT", "This period", "Previous", "All lists"]),
    # "Rows per page" is deliberately absent until there are rows to page.
    ("/addresses", ["Every address across all completed jobs, de-duplicated.",
                    "Total addresses", "Unique domains"]),
    ("/analytics", ["Deliverable rate over time", "Volume by weekday", "Top domains"]),
    ("/exports", ["Slice the workspace and take the CSV.", "Full detail", "Email only",
                  "Export CSV"]),
    ("/history", ["Every job this workspace has run."]),
    ("/settings", ["Engine connection", "Engine status", "SMTP mailbox probe"]),
    ("/how", ["Seven layers, in a fixed order", "What each verdict means"]),
    ("/validate", ["Drop a CSV, or browse", "Upload", "Preview", "Map columns"]),
    ("/single", ["One address, the full seven-layer trace."]),
]
...
def test_empty_workspace_shows_the_designed_empty_states(client):
    assert "Nothing validated yet" in client.get("/addresses").text
...
def test_one_period_of_data_says_so_instead_of_drawing_a_spike(populated):
    ...
    html = populated.get("/dashboard").text
...
def test_contacts_shows_every_verdict_with_its_sub_reason(populated):
    html = populated.get("/addresses").text
...
def test_verdict_filter_narrows_the_list(populated):
    only_risky = populated.get("/addresses?verdict=risky").text
...
def test_filters_that_match_nothing_show_the_filtered_empty_state(populated):
    html = populated.get("/addresses?q=zzzznomatch").text
...
def test_row_detail_returns_the_seven_layer_trace(populated):
    r = populated.get("/addresses/detail", params={"email": "no@gone.test"})
    assert r.status_code == 200
...
def test_row_detail_404s_for_an_unknown_address(populated):
    assert populated.get("/addresses/detail", params={"email": "nope@nope.com"}).status_code == 404


def test_single_check_renders_a_real_trace(client):
    html = client.get("/single", params={"email": "not-an-email"}).text
...
def test_unknown_job_404s(client):
    assert client.get("/history/deadbeef").status_code == 404


def test_export_streams_a_filtered_csv(populated):
    r = populated.post("/exports/download", data={"preset": "risky", "columns": "email"})
    assert r.status_code == 200
...
def test_export_full_detail_carries_the_settling_layer(populated):
    r = populated.post("/exports/download", data={"preset": "all", "columns": "full"})
...
def test_static_assets_are_self_hosted(client):
    ...
    assert "fonts.googleapis.com" not in client.get("/dashboard").text
